...
if(isset($_POST['task_title_btn'])){
	...
	$userid = rtrim($arr, ",");
$sql = "INSERT INTO `task_title` ( `title`, `des`, `type_id`, `startTime`, `endTime`, `responsible`, `author`) VALUES ('$title', '$content', '$taskType', '$startTime', '$endTime','$userid','28')";
	if(mysql_query($sql)){
		$url = "index.php";  
					...
					<?php foreach ($groups as $key) {  ?>
				    	<div class="group">
				    		<div class="group_name">
				            	<span><input type="checkbox" class="input check_all" required><?php echo $key['name']?></span>
				            </div>
				            <ul class="group_list">
				              <?php foreach($member as $mem){ 
				 			  ...
				 			  ?>
				                <li>
				                    <span>
				                        <input type="checkbox" class="input check_this" name="userid_<?php echo $mem['uid'] ?>" value="<?php echo $mem['uid'] ?>" required>
				                        <img alt="" title="<?php echo $mem['username'];?>" data-toggle="username" data-placement="bottom">
				                    </span>
				                </li>
				            <?php }} ?>   
				            </ul>
