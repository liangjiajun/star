	...
	var format_data = function(year,month,user_id){
		...
		$.getJSON('get_plan_data.php',{'year':year,'month':month,'user':user_id},function(data){
			for(var user in data){
				...
				$('.tb[data-user='+user+']').append('<div data-user="'+user+'" class="user" style="overflow:hidden;top:-1px;width:'+nm_w*2+'px;left:-'+nm_w*2+'px">'+user+'</div>');
				for(var row in data[user]){
					for(var dt in data[user][row]){
						...
						$('.tb[data-user='+user+']').css('height',(row*nm_w)+'px');
...
<style>
	*{ padding:0; margin:0; font-family:'Arial','微软雅黑'; box-sizing:border-box;}
	...
	.user{ border:#aaa 1px solid;position:absolute; text-align:center; border-right:none; border-top:none;}
...
<div class="page">
	<span style="color:#ff8400">黄色</span>：指教师指派的任务，<span style="color:green">深绿色</span>：指学生自定义的任务，<span style="color:#666">灰色</span>：指过期未指定状态的任务，<span style="color:#000">黑色</span>：指已完成任务，<span style="color:red">红色</span>：指未完成任务。
</div>
