...
/* 进度条 */
function calc_progress($title, $work){ /*print_r($work);*/
    $results = array();
    foreach($title as $val){
        $count = explode(',', $val['responsible']);
        $n = 0;
        foreach($work as $val3){
          // print_r($val2);

          foreach($val3 as $val2){
              if(in_array($val2['user_id'], $count) && $val2['task_title_id']==$val['id']){
                  $n++;
              }
            }
        }
            // echo $n;
        $results[$val['id']] = floor(percentage(count($count), $n));
    }
    return $results;
}
/*
  计算进度百分比
*/
function percentage($length, $current){
    return $current / $length * 100;
}
/*
  负责人读取
*/
function getRespon($respon){
    $userArray = explode(',',$respon);
    $data = array();
    // print_r($userArray);

    foreach($userArray as $u){
        $sql = "SELECT * FROM user_group_users WHERE user_id = $u";
        $sql  = mysql_query($sql);
        $userGroup = mysql_fetch_array($sql);
        $g =  $userGroup["group_id"];
        
        // print_r($u.'-'.$g.'<br>');
        // if($g){
        $sql="SELECT * FROM user_group WHERE id = $g"; 
        // print_r($sql.'<br>');
        $sql =mysql_query($sql);
        // print_r($sql.'<br>');
        $user_group = mysql_fetch_array($sql);
        $group = $user_group['name'];

        $sql = "SELECT * FROM users WHERE id = ".$u;
        $r = mysql_query($sql);
        $user_name = mysql_fetch_assoc($r);

        $data[$g]['group'] = $group;
        $data[$g]['user'][] = $user_name['username'];
        // }
    }
    return $data;
}
