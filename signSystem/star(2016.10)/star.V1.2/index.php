...
/*
	项目标题输出
*/	
$sql="SELECT * FROM task_title ORDER BY id DESC";
$resultTitle =mysql_query($sql);		
$description ='';
while($title=mysql_fetch_assoc($resultTitle)){
	$description[]=$title;
}
...
/*
	引用进度条
*/
$sql="SELECT DISTINCT task_title_id,user_id FROM task_works";
$works =mysql_query($sql);
$works_num ='';

while($files=mysql_fetch_assoc($works)){
...
?>
		...
		<div class="tab-pane active" id="tab10" >
				...
				<tr>
					...
					<td><?php //echo date('Y-m-d',$d['startTime'])  ?><?php echo dayLeft($d['endTime']) ?></td>
					<td>
					...
					<td>
						...
						<?php  
							...
							if(strpos($res,$user) !== false){
								...
								$sql = "SELECT count(id) FROM task_works WHERE user_id = '$user' AND task_title_id = '".$d['id']."'";
								$r = mysql_query($sql);
								$has = mysql_fetch_assoc($r);
								$has = $has["count(id)"];

								if($has){//提交了
									echo '<button type="button" class="btn gray">项目详情</button>';
								}else{
									echo '<button type="button" class="btn btn-defrult">提交作品</button>';
