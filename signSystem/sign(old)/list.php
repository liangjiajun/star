...
/*拿到本周的签到的日期*/
$colum = $db->fetch($db->query('SELECT * FROM `sign_data` WHERE `sign_data`.end_time <='.mktime(0,0,0,$now_end_explode[1],$now_end_explode[2],$now_end_explode[0]).' AND `sign_data`.start_time >='.mktime(0,0,0,$now_start_explode[1],$now_start_explode[2],$now_start_explode[0]).'  GROUP BY `new_date`'));

$date_all_time = $db->fetch($db->query('SELECT * FROM `sign_data` GROUP BY `new_date`'));

/*

//预计连起来作为搜索条件 2016-11-14 -- 2016-11-20
foreach ($date_all_time as $k => $v) {$date_all_today[]=$v['new_date'];}
$date_all_today  = $date_all_time[0]['new_date'];
$date=date_create($date_all_today);
date_add($date,date_interval_create_from_date_string("6 days"));
$data_time_today = date_format($date,"Y-m-d");

*/


/*签到的用户名单*/
$user = $db->select('users_data',['id','name','username','password'],['show_switch'=>2]);
...
<form method="post" class="right" action="search.php">
	<select name="newDate select-text">
...
<?php foreach ($user as $k1 => $u) { 
	if($u['id']!= 9 && $u['id']!=14 && $u['id']!=15 && $u['id']!=26 && $u['id']!=27 && $u['id']!=28 && $u['id']!=29 && $u['id']!=30 && $u['id']!=31 && $u['id']!=32 && $u['id']!=33 && $u['id']!=34 && $u['id']!=35){
?>	  
	<tr>
	  <td><?= $u['name'] ?></td>
