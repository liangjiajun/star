...
class WebSocket
{
	...
	public function send_user_queue($socket,$nm){
		$dt['event'] = 'queue';
		$dt['nm'] = $nm;
		$dt = json_encode($dt);
		$this->send($socket, $dt);
	...
	public function refresh_user_queue(){
		$i = 0;
		foreach($this->gg_list as $gg){
			$this->send_user_queue($gg,$i);
			$i++;
		}
		$i = 0;
		foreach($this->mm_list as $mm){
			$this->send_user_queue($mm,$i);
			$i++;
		}
	}

	public function queue_disconnect($scoket){
		$key = array_search($scoket, $this->gg_list);
		if ($key !== false){
			array_splice($this->gg_list, $key, 1);
		}
		$key = array_search($scoket, $this->mm_list);
		if ($key !== false){
			array_splice($this->mm_list, $key, 1);
		}
		$this->refresh_user_queue();
	}

	public function friend_disconnect($scoket){
		$dt['event'] = 'friend_disconnect';
		$dt = json_encode($dt);
		$this->send($scoket, $dt);
	}

	public function create_room(&$arr,&$other_arr,$scoket){
		if(count($other_arr) > 0){
			$room_nm = uniqid();
			$this->rooms[$room_nm][] = $scoket;
			$this->rooms[$room_nm][] = $other_arr[0];
			array_shift($other_arr);
			$dt['event'] = 'create_room';
			$dt['room_nm'] = $room_nm;
			$dt = json_encode($dt);
			$this->send($this->rooms[$room_nm][0], $dt);
			$this->send($this->rooms[$room_nm][1], $dt);
			$this->refresh_user_queue();
		}else{
			$arr[] = $scoket;
			$this->send_user_queue($scoket,count($arr)-1);
		}
	}

	public function find_room($scoket){
		$nm = false;
		foreach($this->rooms as $room_nm => $room){
			if($room[0] == $scoket || $room[1] == $scoket){
				$nm = $room_nm;
				break;
			}
		}
	...
	public function find_friend($scoket){
		$friend = false;
		$room_nm = $this->find_room($scoket);
		if($room_nm){
			$friend = $this->rooms[$room_nm][0] == $scoket ? $this->rooms[$room_nm][1] : $this->rooms[$room_nm][0];
		}
		return $friend;
	}
	...
	protected function process($user, $msg)
	{
		...
		switch($data->event){
			case 'new_user':
				if( $data->sex == 'GG'){
					$this->create_room($this->gg_list,$this->mm_list,$user->socket);
				}else{
					$this->create_room($this->mm_list,$this->gg_list,$user->socket);
				}
				break;
			case 'msg':
				$friend = $this->find_friend($user->socket);
				$dt['event'] = 'hesay';
				$dt['msg'] = $data->msg;
				$dt = json_encode($dt);
				$this->send($friend, $dt);
				break;
			case 'disconnect':
				$room_nm = $this->find_room($user->socket);
				$friend = $this->find_friend($user->socket);
				$this->friend_disconnect($friend);
				unset($this->rooms[$room_nm]);
				break;
		}
	}	
	...
	public function connect($clientSocket)
	{
		...
		$this->say($user->socket . " CONNECTED!" . date("Y-m-d H-i-s"));
	}
	...
	public function disconnect($clientSocket)
	{
		...
		{ 
			...
			//用户退出
			$room_nm = $this->find_room($clientSocket);
			$friend = $this->find_friend($clientSocket);
			if($friend){
				$this->friend_disconnect($friend);
				unset($this->rooms[$room_nm]);
			}else{
				$this->queue_disconnect($clientSocket);
			}
...
$web = new WebSocket('121.199.9.26','8080');
$web->run();
